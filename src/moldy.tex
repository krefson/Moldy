...
%%%%%%% RCS Emulation stuff                                 %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\RCSrevision}{$ $Revision: 2.18 $ $}
...
\\Oxford OX1
3PR\\\htmladdnormallink{Keith.Refson@earth.ox.ac.uk}{mailto:Keith.Refson@earth.ox.ac.uk}\\} 
...
\textbf{default} & \textbf{function}  \endfirsthead%\hline\hline 
\caption[]{Control Parameters (\emph{continued})}
\\\hline
\textbf{name} & \textbf{type} &
\textbf{default} & \textbf{function}   \endhead\hline\hline
...
Setting a negative value for \Lit{control.alpha} has the effect of
disabling the Ewald sum entirely.  Any Coulombic interactions are
evaluated in real-space up to the limit set by the cutoff.  (This
happens automatically if all charges are zero).
...
\item[Windows 95/98/NT] The most straightforward way to build \moldy\ is
  ...
  \Lit{\htmladdnormallink{http://sourceware.cygnus.com/cygwin/}%
  {http://sourceware.cygnus.com/cygwin/index.html}}.
...
code is conditionally included. On unix-like systems the ``configure''
script detects the availability of the XDR headers and libraries.  If
``configure'' is not usable the capabilities may be enabled by
defining the C preprocessor macro \Lit{USE\_XDR}.
...
\item[MPI] The MPICH implementation can be downloaded by anonymous ftp
  from the URL
   \Lit{\htmladdnormallink{ftp://info.mcs.anl.gov/pub/mpi/mpich.tar.gz}%
{ftp://info.mcs.anl.gov/pub/mpi/mpich.tar.gz}}\ and with other information at
   \Lit{\htmladdnormallink{http://www.mcs.anl.gov/mpi/index.html}%
{http://www.mcs.anl.gov/mpi/index.html}}.

  The CHIMP implementation can be downloaded by anonymous ftp
  from
   \Lit{\htmladdnormallink{ftp://ftp.epcc.ed.ac.uk/pub/chimp/release/chimp.tar.Z}%
{ftp://ftp.epcc.ed.ac.uk/pub/chimp/release/chimp.tar.Z}}.
\item[TCGMSG] This may be obtained by anonymous ftp from
   \Lit{\htmladdnormallink{ftp://ftp.tcg.anl.gov/pub/tcgmsg/tcgmsg.4.04.tar.Z}%
...
\item[BSP] The Oxford BSP Library is available through Oxford
  Parallel's WWW server
   \Lit{\htmladdnormallink{http://www.BSP-Worldwide.org/implmnts/oxtool.htm}{http://www.BSP-Worldwide.org/implmnts/oxtool.htm}}\ or by anonymous ftp
   \Lit{\htmladdnormallink{ftp://ftp.comlab.ox.ac.uk/pub/Packages/BSP/}%
...
in PDB or XYZ form, suitable for input to the freely available viewers
...
is in SCHAKAL file format. If it is renamed to \Fname{mdxyz} or
\Fname{mdpdb} then the default is XYZ or PDB file format respectively.
