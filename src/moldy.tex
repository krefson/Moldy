...
%%%%%%% RCS Emulation stuff                                 %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\RCSrevision}{$ $Revision: 2.29.4.7 $ $}
...
q_1 & = & \cos \frac{\phi-\psi}{2} \sin \frac{\theta}{2} \nonumber \\
q_2 & = & \sin \frac{\phi-\psi}{2} \sin \frac{\theta}{2} \nonumber \\
...
where the arguments have the following meanings:
\begin{Argdescription}
\item[-d] read the system info and centre of mass velocities from a dump file (of \Lit{dump-level} 
...
where the meanings of the options are
\begin{Argdescription}
\item[-d] read the system info and centre of mass coordinates from a dump file given as a
...
species is replacing an existing polyatomic species, by default the same quaternions (orientation) will be used.
 ...
 options \Lit{-f}, \Lit{-t} or \Lit{-p} are specified. The \Lit{-k} option randomly generates
new quaternions for the substituting molecules regardless of whether the species it replaces is
monatomic or polyatomic. This option is overridden by options \Lit{-f}, \Lit{-t} and \Lit{-p}. The Euler angles follow the same convention as that used in Allen and Tildesley\cite{allen:87}.

Options \Lit{-f}, \Lit{-t} and \Lit{-p} are used to align all polyatomic solute molecules in the same direction, 
...
Euler angles specified with \Lit{-f}, \Lit{-t} and \Lit{-p} are used for the solute molecules.
...
the first site of the solvent molecule. In both cases, the site coordinates of both species in the
header file are written relative to the first site. \textbf{N.B.} It
is possible to add a systematic shift (using \Lit{-h}) or positional disorder (using \Lit{-h} and \Lit{-k}) to dopant positions 
by specifying a structure file that contains only a single atom. This atom's
site coordinate vector (in {\AA}) should correspond to the desired shift vector (relative to the first site 
