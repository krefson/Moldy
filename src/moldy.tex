...
may be performed either in the usual $NVT$ ensemble or in an $N\sigma
...
and Unix(tm) (both BSD and system V varieties).  It should be trivial to
move it to any other machine with a good `C' compiler.
...
including Cray Research Inc, Convex, Stellar, Ardent and Alliant.  On
the cray XMP-48 its performance can exceed 100 MFlop/sec (on a
suitably large system). A experimental version for parallel machines
with shared memory is also available.

\chapter{Algorithms and Equations}
% Disclaimer to purpose and refs here
% Description of nature of models Moldy can handle -
% rigid molecule and site pair forces.
\section{The Equations of Motion}
% Newton-Euler
% Quaternions
\label{sec:quaternions}
% Sonnenschein's 2nd order method
...
\label{sec:ewald}
%Equations
% Self energies & uniform sheet correction
% Surface dipole term
...
model of Matsuoka, Clementi and Yoshimine\cite{MCY75}, 
...
method of Parinello and Rahman\cite{PR81} (see section~\ref{sec:const-stress}).
The value of the ``MD cell'' mass parameter, $W$ is given by the
control parameter \verb'w' and the external pressure by
\verb'pressure'. At present it is not possible to specify an anisotropic
external stress, though this capability may be added in future
...
Smith\cite{DLPS8?}.  See the note in section~\ref{sec:ewald} for an
...
the Kernighan  and Ritchie\cite{KR78} compatible subset  of  ANSI C and
assumes the  library  calls and  header  files   defined for a  hosted
implementation of the standard.  It should therefore be possible
to compile and run \moldy on any computer which has a good C compiler.
...
textbook, \eg Kernighan and Ritchie\cite{KR78} for an explanation of C
pointers and arrays.)  Unfortunately this means that the compiler can
not determine that each iteration of the loop is independant of the
preceding iterations.  In the jargon of vectorizing compilers, there
may be a {\em vector dependency} or {\em recurrence}.  Each
manufacturer's compiler has its own peculiar method to inform it that
despite appearances there is no dependency.  For example the CRAY
...
evaluation of the electrostatic term $\mbox{erfc}(\alpha r) / r$ which is
evaluated by the polynomial expansion in section 7.1.26 of Abramowitz and
Stegun\cite{AS70}. There are currently {\em two} versions of each
loop, the second omitting this term for efficiency when all the
electric charges are zero (which case is flagged by a negative value of
$\alpha$). 
...
the method developed by the Kingston group\cite{C8?}. That is, a loop
of the form
\begin{verbatim}
	for(i = 0; i < N; i++)
	   <body of loop>
...
}\cite{JG??}.  Only the oxygen site interacts via the Lennard-Jones
...
This is a three-component system consisting of MCY water\cite{MCY76},
Magnesium and Chloride ions.  The Mg$^{2+}$ potential was fitted to
the SCF calculations of Dietz and Heinzinger\cite{DH8?} and the Cl$^-$
to the calculations of Kistenmacher, Popkie and Clementi\cite{KPC7?}.
