...
\xdef\RCSrevision{\$Revision: 2.1 $}\xdef\manversA{$\$junk $ $}}
...
particular, 
\begin{list}{}{\labelwidth=1in \labelsep=0.25in \leftmargin=1.5in
...
and {\em dumpextract\/} which extracts atomic or molecular
...
read by {\em dumpextract\/} on any machine.  The control parameter
...
called {\em dumpconvert\/} is provided which converts dump files to a
portable text file format (which may be easily moved between machines)
and back again. It is described in appendix~\ref{sec:dumpconvert}.
...
approach to equilibrium.  {\em Moldyextract\/} processes the periodic
...
\begin{center}
\verb'moldyextract -f' {\em fields output-file1 output-file2} \ldots
\end{center}
...
{\em Moldyextract\/} can currently only extract information from the
...
\section{Dumpanalyze}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{\em Dumpanalyze\/} examines the dump files given as arguments and
prints out the header information to help with identification. For
example
\begin{center}
\verb'dumpanalyze' {\em dump-file1 dump-file2} \ldots
\end{center}
...
{\em Dumpconvert\/} is a tool for moving binary dump files between
...
\begin{center}
\verb'dumpconvert' {\em binary-dump-file text-dump-file} 
\end{center}
...
\begin{center}
\verb'dumpconvert -d' {\em text-dump-file binary-dump-file} 
\end{center}
...
\begin{center}
\verb'dumpconvert -x' {\em native-dump-file xdr-dump-file}. 
\end{center}
...
Dumpextract\/} is a program which processes dump files, extracts a
...
\begin{center}
\verb'dumpextract -R ' {\em nmols\/} \verb' -Q ' {\em nquats\/} 
...
order; {\em dumpextract\/} automatically determines the sequence from
the information in the headers.  This is not as daft as it sounds,
...
record size information to be present.  {\em Moldyextract\/} can not
