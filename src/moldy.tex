...
%%%%%%% RCS Emulation stuff                                 %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\RCSrevision}{$ $Revision: 2.29.4.9 $ $}
...
\author{Keith Refson\\Rutherford Appleton Laboratory\\Didcot
\\Oxfordshire OX11 0QX\\
\htmladdnormallink{k.refson@rl.ac.uk}{mailto:k.refson@rl.ac.uk}\\}
...
UNIX any of
\begin{verbatim}
        % moldy < control.min > output.mot
        % moldy control.min output.mot
        % moldy control.min
\end{verbatim}
will cause moldy to read from the file called \Fname{control.min} and in
the first two examples to write its output to \Fname{output.mot}.  The
...
%been written.  It is possible that following a crash there may be one
...
\begin{center}
\Lit{mdavpos}
{-s} \textit{system-specification} \Lit{|}
{-r} \textit{restart-file}
...
The configurational data is from a dump file given as a prototype name
...
\item[-b] bond limits in hundredths of angstroms (\ie\ picometres) entered in form
  ``minimum-maximum''. Limits may be entered as integers only, so
  smallest difference is 0.01{\AA}. Defaults to 0 to 2{\AA}.
...
\begin{Argdescription}
\item[-b] bond interval limits in hundredths of angstroms (\ie\ picometres) entered in form
  ``minimum-maximum:increment''. Limits may be entered as integers
  only, so smallest increment is 0.01{\AA}. Defaults to 0.0 to the largest bond length in the given time slice, with
  a 0.01{\AA} increment.
\item[-a] angle interval limits in degrees entered in form
  ...
  the largest angle in the given time slice, with a 1{\textdegree}\ increment.
\item[-p] give output as list of type and number
of neighbouring (bonded) species for each molecule.
\item[-z] include bins with zero counts in output.
\item[-o] name of optional output file. Defaults to standard output.
\end{Argdescription}

The input file is a compulsory argument, and must have the same format as the output from an \Fname{mdbond} calculation.
...
\textbf{N.B.} The default smallest binning width of 0.1{\AA} can be altered by re-defining the SCALE parameter in utlsup.h and recompiling \Fname{mdbond} and \Fname{bdist}.
...
The system info and centre of mass velocities are read from a dump file (of \Lit{dump-level}
...
\end{Argdescription}
 ...
 The system info and centre of mass coordinates are read from a dump file, which must always be provided, and can be given as a
...
\item[-a] file containing structural information for polyatomic solute species. Can be in CSSR, PDB, XYZ, XTL, SHELX 
  or SCHAKAL format.
\item[-e] read the species data (mass, charge, symbol) corresponding to the
solute species (atom) name from the file specified.
Overridden  by options \Lit{-w}, \Lit{-q}, \Lit{-z}.
...
\begin{Argdescription}  
\item[-f] Euler angle \begin{ensuremath}\phi\end{ensuremath} for polyatomic ''solute'' molecules (in degrees).
\item[-t] Euler angle \begin{ensuremath}\theta\end{ensuremath} for polyatomic ''solute'' molecules (in degrees).
\item[-p] Euler angle \begin{ensuremath}\psi\end{ensuremath} for polyatomic ''solute'' molecules (in degrees).
...
of Cambridge CSSR, Brookhaven PDB, Insight XTL, Schakal, Shelx or generic XYZ format to be specified. If the new polyatomic
species is replacing an existing polyatomic species, by default the same quaternions (orientation) will be used.
 ...
 options \Lit{-f}, \Lit{-t}, \Lit{-p} or \Lit{-j} are specified. Options \Lit{-f}, \Lit{-t} and \Lit{-p} are used to input Euler angles \begin{ensuremath}\phi\end{ensuremath}, \begin{ensuremath}\theta\end{ensuremath} and \begin{ensuremath}\psi\end{ensuremath}, respectively, thereby aligning all polyatomic solute molecules in the same direction (see equations~\ref{eqn:qeuler}).
...
It is possible to add a systematic shift (using \Lit{-h}) or positional disorder (using \Lit{-h} and \Lit{-j}) to dopant positions 
...
The properties of monatomic solute species not already in the input file can be entered explicitly on the command
line using options \Lit{-w}, \Lit{-q}, \Lit{-z}, otherwise standard values for each element are used.
In the case of polyatomic solutes, these options are ignored and the species components
matched with pre-existing components in the original structure.
...
Alternatively, values can be read from a species data file using option \Lit{-e}.
The format of a species data file is
{\textit{species-name}$_{i}$ %
                  \textit{symbol}$_{i}$ \textit{mass}$_{i}$} \textit{charge}$_{i}$
where $mass_{i}$ is in amu, and $charge_{i}$ is in units of electronic charge.
\textbf{N.B.} The maximum number of species that can be
...
masses of unrecognized sites will be assigned a value of zero. 
...
However, the outputted potential parameters may vary slightly from the original
...
section~\ref{sec:sys-spec} for further details. 
...
PDB, Schakal, Insight XTL, Shelx or XYZ structural data files. This utility
...
(\eg\ when reading from an XYZ file or CSSR files written in non-periodic mode),
...
structure files are automatically assigned standard values for that element.
Alternative values can be specified using option \Lit{-e} with a species data file. If a potential
...
Further details about the species data
and potential parameters files can be found in appendix~\ref{sec:ransub}.
...
\Lit{[-a} \textit{a-direction-repeat-factor}\Lit{]}
\Lit{[-b} \textit{b-direction-repeat-factor}\Lit{]}
\Lit{[-c} \textit{c-direction-repeat-factor}\Lit{]}
\Lit{[-o} \textit{output-file}\Lit{]}
\end{center}
...
\item[-i] read the structural data from given file. The file can be in
 Brookhaven PDB, Cambridge CSSR, Insight XTL, Shelx, XYZ or Schakal format.
...
\item[-a] repeat factor for $a$ parameter of basic cell. Default is 1.
\item[-b] repeat factor for $b$ parameter of basic cell. Default is 1.
\item[-c] repeat factor for $c$ parameter of basic cell. Default is 1.
\item[-o] name of output file. Defaults to standard output.
\end{Argdescription}
...
``\Fname{.shak}'', ``\Fname{.xtl}'', ``\Fname{.ins/.res}'' or ``\Fname{.xyz}'' (upper- or lowercase), so that \emph{syswrite} can
